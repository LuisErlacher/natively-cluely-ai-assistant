    ...
    - **Skill deletion in Settings → Skills**: User-installed skills (e.g. `code-simplifier`) now have a per-row delete control — hover-reveal Trash2 icon, also revealed on keyboard focus for accessibility. A native `confirm("Delete \"<name>\"? This cannot be undone.")` guard matches the existing `AIProvidersSettings` delete pattern. Built-in skills (e.g. `humanize-ai-text`) keep their "Built-in" badge and cannot be deleted — they are protected at three layers: (1) no delete button rendered in the UI, (2) the `SkillsManager.deleteSkill()` method rejects with a clear error if a direct IPC call slips through, (3) `ensureBuiltinSkills()` would silently re-seed the folder anyway. Per-skill in-flight tracking via `deletingIds: Set<string>` prevents double-click races on the same skill. The delete itself is TOCTOU-safe: `SkillsManager.deleteSkill()` calls `fs.realpathSync` on both the target and `skillsDir` at delete-time, then `fs.rmSync({recursive:true,force:true})` — meaning a symlink-swap between the initial `loadSkills()` read and the delete (an attack vector that would let `rmSync` follow a symlink out of the skills dir) is closed by the realpath containment check. Sanitized error messages ("Could not delete skill.") — no internal-implementation detail leaks to the renderer banner. (`76d531b` — 8 files, +874/-30.) See the **Code Review Fixes (2026-07-06)** section below for the senior review findings that shaped this and the follow-up classification hardening fix.
